...
Each appears only when your project shows evidence of that tool, never auto-runs, and caches its last verdict — so Salience answers "what did PHPStan say last time?" without running anything.
...
Salience also flags problems with the runtime itself: an extension your project needs that isn't declared or isn't loaded, lockfile drift, a missing binary. It tints red when an issue is high severity, amber when it can wait.
