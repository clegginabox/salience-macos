...
- **No write access.** Connectors fetch state — they never open, close, comment on, edit, or merge anything. GitHub tokens only need read access. A [fine-grained token](/docs/integrations/github#before-you-connect) is the narrowest option, limited to the repositories you choose.
...
- **Jira is endpoint-allowlisted.** On Jira Cloud sites (`*.atlassian.net`), Salience can only call a fixed list of read-only endpoints. A request to anything else is refused by the client itself — it's not a convention, it's a wall.
- **AWS comes with its own policy.** The **Permissions** card lists every AWS action Salience can call, each with the reason it's needed. That's ten read-only actions across STS, ECR and ECS, listed in full on the [AWS](/docs/integrations/aws#permissions) page. There are no write, delete or deploy actions. **Copy least-privilege policy** copies an IAM policy with exactly those actions. Attach it to a dedicated role and Salience never needs a broad profile.
...
Salience makes outbound HTTP calls to the tools you've connected (GitHub, Jira, AWS and the Sentry connector), plus Sentry while crash reporting is enabled. Docker and Git run as local commands and show up in the [command monitor](#command-monitor) instead. To audit them with your own eyes, open **Network Inspector** in the sidebar. Every request the app makes is listed there: destination, method, response status, timing.
