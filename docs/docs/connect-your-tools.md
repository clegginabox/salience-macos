...
Issues for scoped projects show up on the Timeline and Overview, and error paths render on the map.
...
- The **Stack** tab on the Build page — one row per container with its state, health, and first port, plus per-service and whole-stack controls
...
- The map, where each declared service is a building whose power state comes from the matched running container. The right-click command wheel there copies `docker compose` commands to your clipboard rather than running them.
